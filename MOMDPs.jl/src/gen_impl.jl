gen(m::MOMDP, s, a, rng) = NamedTuple()
...
@generated function genout(v::DDNOut{symbols}, m::MOMDP, s, a, rng) where symbols

    # use anything available from gen(m, s, a, rng)
    expr = quote
        x = gen(m, s, a, rng)
        @assert x isa NamedTuple "gen(m::MOMDP, ...) must return a NamedTuple; got a $(typeof(x))"
    end
...
function sorted_deppairs(m::Type{<:MOMDP}, symbols)
    deps = Dict(:s => Symbol[],
                :a => Symbol[],
                :sp => [:s, :a],
                :o => [:s, :a, :sp],
